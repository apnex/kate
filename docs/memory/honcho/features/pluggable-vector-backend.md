...
- **No in-process / Postgres-pgvector backend present at this SHA.** ~~LanceDB (embedded) and Turbopuffer (managed cloud) are the only options.~~ **CORRECTED (batch 3):** a third backend exists — `VECTOR_STORE.TYPE == "pgvector"` keeps embeddings inside the primary Postgres database via SQLAlchemy/pgvector (no separate ABC implementation needed because it uses Postgres directly through `models.Document.embedding`) [code: `src/crud/document.py:468-471`]. The `VECTOR_STORE.MIGRATED` flag enables dual-write to both pgvector and external store during migration. **Three backend modes**: pgvector (single-database simplicity), LanceDB (embedded but separate file), Turbopuffer (managed cloud). The ABC abstracts only the two *external* backends; pgvector uses a different code path. See `04-assessment.md §A26` (amended).
